...
drop_pre = "../../" + cwd.split('/')[-2]+"/"
n = Node(hostsite)
ardustat_id = 9998
known_resistor = 10000 #ohms

print n.sread() #just to make sure things are working.
sleep(1)
...
def calibrate(known_res,ardustat_id):
    ...
    for i in range(0,10):
        ...
        for y in range(0,256):
            ...
            open(drop_pre + 'resistance_tables/resistance_table_'+str(ardustat_id)+'.txt','a').write(str(values['pot_step'])+',' + str(built_in_res) + '\n')
    ...
    pickle.dump(final_dict,open(drop_pre + 'resistance_tables/resistance_table_'+str(ardustat_id)+'.p',"wb"))
    return final_dict
