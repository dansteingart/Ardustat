from pithy import *
print "yeah nah, greg is still a bogan ah."
...
#go through and delete things - have this as main class that is called for communicationa and parsing. Have seperate things for calibrations, cvs, cycler.
...
class Communication():
    ...
    #clean this up you lazy slob
#GO,1408108459939,102,351,0,255,0,0,s0000,99,102,524,ST
...
#this is ugly but it doesn't matter
...
#make sure this is compatible with when I create the resistance tables...
    ...
    def potentiostat(self,potential):
        ...
        return voltage #not sure why I have this here but there must be a reason
    ...
    def galvanostat(self,current,res_table):
        ...
        if current == 0: # make this more robust cause right now this is bs...
            current = 0.001
        R_goal = 1/current
        R_real = 10000
        R_set = 0
...
#cycling stuff
#----------------------------------------------------------------------------#
#TODO: current can be much lower than 5 mA - so should probably work out what resolution to do this at.. Probably best thing to do is to make it just work for mylad and then fuck it all away.
#constant ramp of current until a certain voltage is achieved
#TODO: see if this goes the right way for mylad.. Should probably message him.
    def gradual_charge(self, cycler_dict_gradual_cycle, cycle, res_table):
        ...
        voltage = voltage_limit - 1 #this is a hack and is yuck
        rate = d['rate']
        #TODO: check that this is right
        print "start time_diff ", time_diff
        print "time_limit ", time_limit
        print "start voltage ", voltage
        print "start voltage limit ", voltage_limit
        while (time_diff < time_limit and abs(voltage) > abs(voltage_limit)):
             
            print "attempt current: %r" %current
            step_time = time.time()
            self.galvanostat(current,res_table)
    ...
    #include constant ramp of current up to a certain voltage
