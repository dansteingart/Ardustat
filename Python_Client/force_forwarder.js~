...
function write2CSV(chunk) {
	...
	if(count==0) {
		...
		json2csv({data: headers, fields: ['0','1','2','4'], hasCSVColumnTitle: false}, function(err, csv) {
		    ...
		    while(orig.indexOf("GO")!=-1 && orig.indexOf("ST")!=-1) {
                ...
                if (parseInt(chunks[0]) < 10000) {
			        ...
			        if(parseInt(chunks[0])>parseInt(lastTime)) {
				        ...
				        json2csv({data: foo, fields: ['0','1','2','3'], hasCSVColumnTitle: false}, function(err, csv) {
					        if (err) console.log(err);
