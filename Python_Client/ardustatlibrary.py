		...
		if parsedict["success"] == True:
			if initfileio == True:
				...
				if initfileio == True:
					...
					parseddatafile.write("Unix Time,DAC 0 Setting (V),DAC 0 Reading (ADC 1) (V),Cell Voltage (ADC 0) (V),Potentiometer Setting (0-255),Potentiometer Setting (Ohms),GND Reading (ADC 2) (V),Current Calculation (A),Cell Resistance Calculation (Ohms),Reference Electrode Reading (ADC 3) (V),Mode,Setting (V),Last Command Received,Calibrated,Cell Voltage Reading - Reference Electrode Reading (V),Value_v_ref (V),Value_cell_voltage (V)\n")
				initfileio = False
			...
			try:
				...
				parsestring = d("time")+d("DAC0_setting")+d("DAC0_ADC")+d("cell_ADC")+d("pot_step")+d("resistance")+d("GND")+d("current")+d("cell_resistance")+d("reference_electrode")+d("mode")+d("setting")+d("last_command").replace("\0","")+d("calibration")+d("cell_ADCminusreference_electrode")[:-1]+d("value_v_ref")+d("value_cell_voltage")+"\n"
...
def parse(reading,id=None):
	...
	if reading.find("GO") == 0 and reading.find("ST") and reading.rfind("GO") == 0:
		...
		outdict['value_v_ref'] = outdict['cell_ADC']-outdict['reference_electrode']
		outdict['value_cell_voltage'] = outdict['cell_ADC']-outdict['GND']
