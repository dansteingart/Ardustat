...
var server = new Server("127.0.0.1", 27017, {auto_reconnect: true, poolSize:1, native_parser:false});

// Read in the test file
var fileData = require('fs').readFileSync("./iya_logo_final_bw.jpg");

// ------------------------------------------------------------------------------
// TEST MONGODB NATIVE
// ------------------------------------------------------------------------------
// Open a db for the file
//new Db('gridfs_benchmark1', server, {}).open(function(err, new_client) {
//    new_client.dropDatabase(function(err, result) {
//        new_client.close();

//        new Db('gridfs_benchmark1', server, {}).open(function(err, client) {
...
var executeMongolianTest = function() {
    var db = new Mongolian('mongo://localhost/gridfs_benchmark2', { log:false })
    db.dropDatabase(function() {
        var gridfs = db.gridfs()

        gridfs.findOne('foo', function() {
            currentWritingIndex = 0;
            // Start Time
            var startTime = new Date().getTime();
            ...
            for (var i = 0; i < COUNT; i++) {
                var stream = gridfs.create("foobar" + i).writeStream();
