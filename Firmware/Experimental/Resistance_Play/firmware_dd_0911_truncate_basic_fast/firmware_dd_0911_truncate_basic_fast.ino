...
//  Serial.println(dac);
  ...
  //Serial.print(", wept ");
  //Serial.println(wept);
  if ((wepcounter >= wepcounterlimit) || (res_last <= res_last_limit))
  {
...
//just checking to see if this makes a difference
void res_truncate_fix() {
  dac = analogRead(1);
  adc = analogRead(0);
  float restimator = res*40 + 100; //estimate of resistance
  //Serial.print("dac ");
  //Serial.print(dac);
  //Serial.print(", adc ");
  //Serial.print(adc)
   
  float est_current = ((dac) - (adc))/float(restimator);
  res = res + 1;
  res = constrain(res,1,255);
  write_pot(pot,resistance1,res);
  restimator = res*40 + 100;
  outvolt = adc + est_current * restimator;
 //Serial.print(" outvolt ");
 //Serial.println(outvolt);
}
